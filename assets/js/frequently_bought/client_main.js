function wceazy_frontend_pf_rating_changed(view) {
    if (jQuery(view).hasClass("active")) {
        jQuery(view).parent().find(".wceazy_pf_frequently_bought_item").removeClass("active")
    } else {
        jQuery(view).parent().find(".wceazy_pf_frequently_bought_item").removeClass("active")
...
function wceazy_pf_add_to_cart(view, product_id) {
    ...
    var post_data = {
        'action': 'wceazy_frequently_bought_add_to_cart',
        'product_id': product_id
    };

    jQuery.ajax({
        url: wceazy_client_frequently_bought_object.ajaxurl,
        type: "POST",
        data: post_data,
...
function wceazy_frontend_pf_search(page = "0") {
    ...
    if (jQuery(".wceazy_pf_frequently_bought").length > 0) {
        rating = jQuery.isNumeric(jQuery(".wceazy_pf_frequently_bought .wceazy_pf_frequently_bought_item.active").attr("data-rating")) ? jQuery(".wceazy_pf_frequently_bought .wceazy_pf_frequently_bought_item.active").attr("data-rating") : rating;
    }
    ...
    let post_data = {
        'action': 'wceazy_frequently_bought_search',
        'page': page,
        'query': search_query,
